...
- SSU validation and build_ssu directive: all contract types that reference an SSU (`coin_for_item`, `item_for_coin`, `item_for_item`) require a valid storage unit on the network node. `ResolveIntent` returns `ErrNoSSU` if `snapshot.NodeSSUs` contains no entries with non-empty, non-zero ObjectIDs. Before any contract generation in `attemptContractFill`, the engine checks `HasValidSSU(snapshot)`. If no SSUs exist, it emits a single `build_ssu` UI-only directive (no on-chain transaction) instructing the player to deploy a storage unit. The directive uses a deterministic contract ID (`build_ssu_<cormID_prefix>`) tracked in a per-corm map to prevent duplicates. When SSUs later appear on the network node, the build_ssu directive is auto-completed and an announcement is sent to the corm log. `GetNodeSSUs` currently returns seed data in seed mode and nil otherwise (pending indexer/RPC integration); in non-seed environments this means the build_ssu directive is emitted until real SSU discovery is implemented.
...
- **`internal/reasoning/ssu_test.go`** — unit tests for SSU validation (`HasValidSSU` with empty/zero/valid/mixed SSU lists), `ResolveIntent` `ErrNoSSU` for all SSU-dependent contract types, `build_ssu` bypass, zero-address SSU rejection, narrative helpers, and deterministic contract ID generation.
...
- SSU validation: `HasValidSSU`, `ErrNoSSU` from `ResolveIntent`, `build_ssu` intent bypass, zero-address SSU rejection, build_ssu narrative and auto-completion announcement
