...
- **Testnet:** `make upgrade-contracts ENV=stillness` (all packages) or `make upgrade-contracts ENV=stillness PKGS="corm_state corm_auth"` (specific packages)
...
Shared objects use two versioning patterns:

- **Struct-field version** (`tribe`): `TribeRegistry` and `Tribe` carry a `version: u64` field set at creation. All mutating functions assert `version == CURRENT_VERSION`. Migration functions bump the field.
- **Dynamic-field version** (`corm_auth`, `corm_state`, `corm_coin`): These packages were published before version fields were added. Since Sui forbids struct layout changes on compatible upgrades, version is tracked via a `VersionKey` dynamic field on each shared object's UID. The `assert_version()` helper defaults to version 1 for pre-upgrade objects (backwards-compatible). Migration functions stamp or bump the dynamic field. Affected objects: `WitnessRegistry`, `CormState`, `CormConfig`, `CoinAuthority`.
