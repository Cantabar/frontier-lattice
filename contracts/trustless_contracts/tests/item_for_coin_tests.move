...
// =========================================================================
// Non-owner poster: offers items from player inventory, receives coins
// =========================================================================

/// Player B (filler_id/user_b) has items in their player inventory on
/// Player A's SSU. Player B creates an ItemForCoin contract. A filler
/// pays coins. Verify items release to filler and coins go to Player B.
#[test]
fun test_nonowner_poster_item_for_coin() {
    let mut ts = ts::begin(@0x0);
    // poster_id = user_a (SSU owner), filler_id = user_b (non-owner poster)
    let (poster_id, filler_id) = test_helpers::setup_ssu_characters(&mut ts);
    let (storage_id, nwn_id) = test_helpers::create_test_ssu(&mut ts, poster_id);
    test_helpers::online_test_ssu(&mut ts, user_a(), poster_id, storage_id, nwn_id);
    ...
    // Mint items to NON-OWNER's (filler_id/user_b) player inventory
    test_helpers::mint_items_for_character(&mut ts, user_b(), filler_id, storage_id);

    // Non-owner (user_b) withdraws from their player inventory and creates contract
    ts::next_tx(&mut ts, user_b());
    {
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let mut character = ts::take_shared_by_id<Character>(&ts, filler_id);
    ...
    // Verify items are in open inventory (escrowed by contract)
    ts::next_tx(&mut ts, user_a());
    {
        let storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let open_key = storage_unit.open_storage_key();
    ...
    // SSU owner (poster_id/user_a) fills with coins
    ts::next_tx(&mut ts, user_a());
    {
        let mut contract = ts::take_shared<ItemForCoinContract<FILL>>(&ts);
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let filler = ts::take_shared_by_id<Character>(&ts, poster_id);
        let fill_coin = coin::mint_for_testing<FILL>(test_helpers::wanted_amount(), ts::ctx(&mut ts));
        let clock = clock::create_for_testing(ts::ctx(&mut ts));
    ...
    // Verify filler (poster_id) received items in their owned inventory
    ts::next_tx(&mut ts, user_a());
    {
        let storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let filler_char = ts::take_shared_by_id<Character>(&ts, poster_id);
        let filler_cap_id = filler_char.owner_cap_id();
        assert!(storage_unit.item_quantity(filler_cap_id, test_helpers::item_type_id()) == test_helpers::item_quantity());
        ts::return_shared(filler_char);
        ts::return_shared(storage_unit);
    ...
    // Verify NON-OWNER poster (user_b) received coins
    ts::next_tx(&mut ts, user_b());
    {
        let payment = ts::take_from_sender<Coin<FILL>>(&ts);
        assert!(payment.value() == test_helpers::wanted_amount());
...
// =========================================================================
// Non-owner poster cancel: items returned to non-owner's player inventory
// =========================================================================

#[test]
fun test_nonowner_poster_cancel_item_for_coin() {
    let mut ts = ts::begin(@0x0);
    let (poster_id, filler_id) = test_helpers::setup_ssu_characters(&mut ts);
    let (storage_id, nwn_id) = test_helpers::create_test_ssu(&mut ts, poster_id);
    test_helpers::online_test_ssu(&mut ts, user_a(), poster_id, storage_id, nwn_id);
    ...
    // Mint items to non-owner's player inventory
    test_helpers::mint_items_for_character(&mut ts, user_b(), filler_id, storage_id);

    // Non-owner creates ItemForCoin
    ts::next_tx(&mut ts, user_b());
    {
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let mut character = ts::take_shared_by_id<Character>(&ts, filler_id);
    ...
    ts::next_tx(&mut ts, user_b());
    {
        ...
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        item_for_coin::cancel(
            contract, &nonowner, &mut storage_unit, ts::ctx(&mut ts),
        );
        ts::return_shared(nonowner);
        ts::return_shared(storage_unit);
    };

    // Verify items returned to NON-OWNER's player inventory
    ts::next_tx(&mut ts, user_b());
    {
        let storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let nonowner_char = ts::take_shared_by_id<Character>(&ts, filler_id);
        let cap_id = nonowner_char.owner_cap_id();
        assert!(storage_unit.item_quantity(cap_id, test_helpers::item_type_id()) == test_helpers::item_quantity());
