...
fun test_expire_transport_forfeits_stake() {
    ...
    // Create transport — poster withdraws items and locks them
    ts::next_tx(&mut ts, user_a());
    {
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
    ...
    ts::next_tx(&mut ts, user_b());
    {
        ...
        trustless_contracts::accept_transport(
...
fun test_transport_create_and_accept() {
    ...
    // Create transport — poster withdraws items and locks them in open inventory
    ts::next_tx(&mut ts, user_a());
    {
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let mut character = ts::take_shared_by_id<Character>(&ts, poster_id);
        ...
        trustless_contracts::create_transport<ESCROW, FILL>(
            &character, escrow, &mut storage_unit, item, dest_ssu_id,
            stake_amount, false, FAR_FUTURE_MS,
            vector[], vector[], &clock, ts::ctx(&mut ts),
        );
    ...
    // Verify items are in open inventory and contract fields
    ts::next_tx(&mut ts, user_b());
    {
        ...
        assert!(trustless_contracts::contract_target_quantity(&contract) == (ITEM_QUANTITY as u64));
        assert!(trustless_contracts::contract_status(&contract) == trustless_contracts::status_open());
        ts::return_shared(contract);
    };
    ...
    ts::next_tx(&mut ts, user_b());
    {
        ...
        assert!(storage_unit.item_quantity(courier_cap_id, ITEM_TYPE_ID) == ITEM_QUANTITY);
...
fun test_not_divisible_transport() {
    ...
    // Withdraw items and attempt create_transport with indivisible stake
    ts::next_tx(&mut ts, user_a());
    {
        let mut storage_unit = ts::take_shared_by_id<StorageUnit>(&ts, storage_id);
        let mut character = ts::take_shared_by_id<Character>(&ts, poster_id);
