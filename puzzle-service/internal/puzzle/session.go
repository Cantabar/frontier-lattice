...
// AIContract represents a Phase 2 contract generated by the corm-brain AI.
...
type Session struct {
	...
	// Contracts (Phase 1: base contract discovery)
	Contracts        []Contract
	ActiveContractID string // which contract is currently being puzzled

	// AI Contracts (Phase 2: corm-generated directives)
	AIContracts      []AIContract
	PatternAlignment float64 // running pattern alignment score (0.0–1.0)
	CompletedPatterns int    // count of pattern-aligned completions
...
// TransitionToPhase2 moves the session from Phase 1 to Phase 2.
func (s *Session) TransitionToPhase2() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.Phase = PhaseContracts
	s.AIContracts = nil
}

// AddAIContract adds an AI-generated contract to the session.
func (s *Session) AddAIContract(c AIContract) {
	s.mu.Lock()
	defer s.mu.Unlock()
	c.CreatedAt = time.Now()
	s.AIContracts = append(s.AIContracts, c)
}

// UpdateAIContract updates the status of an AI contract by ID.
// Returns true if the contract was found.
func (s *Session) UpdateAIContract(id, status string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	for i := range s.AIContracts {
		if s.AIContracts[i].ID == id {
			s.AIContracts[i].Status = status
			return true
		}
	}
	return false
...
// ActiveAIContracts returns all AI contracts with status "active".
func (s *Session) ActiveAIContracts() []AIContract {
	var out []AIContract
