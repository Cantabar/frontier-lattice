...
- **Deferred Auth / AuthPromptModal** (`components/shared/AuthPromptModal`) — pages that require Location Network auth (Structures, Locations) no longer trigger `getAuthHeader()` on mount. Instead, they render an idle state with a call-to-action button. Clicking it shows the `AuthPromptModal`, which explains *what* will happen (wallet signing prompt) and *why* (identity verification — no funds transferred) before the user confirms. Only after confirmation does `getAuthHeader()` fire and the wallet popup appear. On the Structures page this is an inline banner ("Show Locations"); on the Locations page it is a full-page empty state ("Connect to Location Network"). `useStructureLocationIds` no longer auto-fetches on mount — callers must trigger `refetch()` explicitly after auth.
...
- **Blueprints Hook** (`hooks/useBlueprints`) — loads blueprint data from `/blueprints.json`, converts to `RecipeData[]` for the optimizer. Also exposes `allRecipesMap: Map<typeId, BlueprintRecipe[]>` preserving all blueprint alternatives per output item (including `blueprintId`, `facilityName`, `facilityFamily`) for the per-node blueprint selector. Populates `secondaryOutputs` on `RecipeData` from blueprint entries with multiple outputs (25 of 221 blueprints are multi-output). **Ore-efficient multi-output routing:** multi-output refining recipes are registered as alternatives for ALL their outputs, not just the primary. For each secondary output, a "reoriented" `BlueprintRecipe` is created where that output is promoted to primary (correct run math) and all other outputs become `secondaryOutputs` (byproduct crediting). Alternatives are sorted by crafting style → non-salvage-first → ore efficiency (output per ore unit, descending). This ensures the default recipe for a given mineral uses the most ore-efficient source — e.g. Feldspar (0.75 SG/ore) is preferred over Platinum-Palladium Matrix (0.25 SG/ore) for Silica Grains, avoiding wasted Palladium off-runs.
- **Optimizer Hook** (`hooks/useOptimizer`) — browser-side recipe tree resolution and gap analysis. Resolves a target item + quantity to a full dependency tree, collects leaf materials, and compares against inventory for a shopping list. **Intermediate-aware:** accepts the SSU inventory during resolution and stops recursing into nodes whose required quantity is fully covered by on-hand stock, marking them `satisfiedFromInventory`. Partially-covered intermediates reduce the remaining quantity before expanding children. Accepts a `RecipeLookup` callback so callers can inject per-node blueprint selections. **Multi-output aware:** when a recipe has `secondaryOutputs` (byproducts), the resolver credits produced byproducts into the working inventory after computing runs, so downstream/sibling nodes can consume them instead of requesting redundant processing. Each craftable `ResolvedNode` exposes a `byproducts` array for display.
- **Ore Optimizer** (`lib/oreOptimizer.ts`) — post-pass ore optimization for multi-output refining recipes. `buildByproductIndex` creates a reverse lookup from output typeId to refining recipes that produce it. `optimizeOreUsage` groups leaf-level material demands by their shared refining recipe and computes the minimum ore runs to satisfy all co-products simultaneously (runs = max(ceil(needed_i / perRun_i))), surfacing surplus byproducts. Returns an `OreSummary` with per-ore-type breakdowns and unoptimized (single-output) materials.
- **Blueprint Selection** (`OptimizerPanel`) — when multiple blueprints produce the same output, each craftable tree node shows a dropdown to choose which blueprint/facility to use. Changing a selection re-runs the resolver with the updated recipe. State stored as `Map<outputTypeId, blueprintId>`. **Ore preference:** alternatives are sorted so ore-based (non-salvage) recipes come first; salvage-input recipes (Salvaged Materials 88764, Mummified Clone 88765) are pushed to the end. The default selection (`alternatives[0]`) therefore prefers ore. **Inventory-aware salvage override:** when no user selection exists and the SSU inventory contains salvage materials, the `recipeLookup` callback automatically prefers the salvage recipe so on-hand salvage is consumed rather than planning additional mining.
- **Crafting Style Preference** (`useCraftingStyle`, `OptimizerPanel`) — a persisted Field/Base toggle that controls the default recipe route. "Field" mode (default) sorts Field Refinery and Field Printer recipes first; "Base" mode sorts base-scale facilities (Refinery, Heavy Refinery, Printer, Mini Printer, Heavy Printer) first. Stored in `localStorage` (`frontier-corm:craftingStyle`). Switching the toggle clears per-node blueprint overrides and re-runs the optimizer so the entire tree reflects the new preference.
- **Visual Dependency Tree** (`OptimizerPanel`) — replaces the flat indented-text tree with a structured visual layout: 24×24px item icons, CSS `::before`/`::after` pseudo-element connector lines (L-shaped + vertical rails), depth-tier color coding (cyan → green → violet → amber → coral → teal), `L{n}` depth labels, facility pills, inventory-satisfied badges (✓ IN SSU / partial SSU with dimmed styling), collapse/expand toggles per craftable node, and byproduct badges showing secondary outputs (e.g. "+15 Silica Grains") on multi-output crafting nodes.
- **Ore Summary** (`OptimizerPanel`) — toggleable "Ore Summary" section below gap analysis. When active, groups leaf materials by their shared refining recipe, shows minimum ore units to mine per ore type, per-product breakdown (needed vs. produced), and surplus byproduct quantities. Computed on demand via `optimizeOreUsage` post-pass.
...
- Forge Planner with blueprint browser, recipe tree optimizer (intermediate-aware, multi-output-aware with byproduct crediting, per-node blueprint/facility selection), visual dependency tree with icons, connector lines, and byproduct badges, gap analysis with toggleable ore summary for mining optimization, and multi-input order management
