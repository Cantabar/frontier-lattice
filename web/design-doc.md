...
- **Location Crypto** (`lib/locationCrypto.ts`) — Poseidon4 hash commitment, AES-256-GCM encrypt/decrypt with TLK/PLK, X25519 key unwrap (ECIES), signature-derived X25519 keypair (deterministic from wallet's `signPersonalMessage`), human-readable wallet auth challenge construction ("CORM Location Network — Identity Verification" format).
...
- **Location PODs Hook** (`hooks/useLocationPods`) — fetches, decrypts, submits, and revokes location PODs. Handles TLK/PLK initialization (including solo mode via `initializeSoloPlk`), wrapped key fetching, Network Node POD registration and refresh. Auto-detects solo mode via synthetic `solo:<address>` tribe IDs and uses the `GET /solo` endpoint for solo fetches. Auth fallback chain: (1) check cached `Bearer` session token, (2) try `signPersonalMessage` → `SuiSig` header, (3) if that fails fall back to `signTransaction` → `TxSig` header (universally supported — every Sui wallet must implement transaction signing), (4) exchange whichever auth header for a session token via `POST /locations/session`. The `TxSig` fallback builds a minimal no-op transaction (split 0 SUI + self-transfer), signs it with the wallet, and sends the challenge + signed tx bytes to the indexer. Caches the `Bearer` token in a module-level variable + `sessionStorage` (survives navigations, cleared on tab close). Falls back to direct `SuiSig`/`TxSig` auth if session creation fails.
